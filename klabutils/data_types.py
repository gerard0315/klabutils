...
from klabutils.util import has_num
from klabutils.compat import tempfile
...
def data_frame_to_json(df, run, key, step):
    ...
    if len(missing_reqs) > 0:
        raise klabutils.Error("Failed to save data frame. Please run 'pip install %s'" % ' '.join(missing_reqs))
