...
elif os.path.exists(os.path.join(env.get_dir(os.getcwd()), 'klabuils')):
    __stage_dir__ = "klabuils" + os.sep
else:
    __stage_dir__ = None
...
def wandb_dir():
    ...
    path = os.path.join(root_dir, __stage_dir__ or ("klabuils" + os.sep))
    if not os.access(root_dir, os.W_OK):
        termwarn("Path %s wasn't writable, using system temp directory" % path)
        path = os.path.join(tempfile.gettempdir(), __stage_dir__ or ("klabuils" + os.sep))
    return path
